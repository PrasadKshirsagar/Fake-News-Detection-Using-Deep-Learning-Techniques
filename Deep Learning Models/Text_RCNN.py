...
#Reading the data
df2 = pd.read_csv('../input/btp-dataset/kaggle_train.csv')
X = df2.iloc[:, 3:4].values
X_train = []
for i in X:
    for j in i:
        X_train.append(str(j))
y_train = df2.iloc[:, 5].values
...
semantic = Conv1D(hidden_dim_2, kernel_size = 3, activation = "tanh")(dropout_1) # See equation (4).
...
history = model.fit([np.array(X_train), np.array(left_context_as_array), np.array(right_context_as_array)], y_train, epochs = 5, verbose = 1, batch_size = batch_size, validation_split=0.1)
...
dataset2 = pd.read_csv('../input/btp-dataset/kaggle_test.csv')

X_test_init = dataset2.iloc[:, 3:4].values
